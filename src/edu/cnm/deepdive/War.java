...
public class War {
  ...
  public void play() {
    while (!hand1.isEmpty()) {
      ...
      System.out.printf("Player1: %s\tPlayer2: %s\t", card1, card2);
      if (comp.compare(card1, card2) > 0) {
        tally1 += 2;
        ...
        while(comparison == 0 && !hand1.isEmpty()) {
          ...
          for (int i = 0; i < turn; i++) {
            ...
            pile += 2;
            System.out.printf("Player1: %s\tPlayer2: %s\t%n", card1, card2);
          }
        ...
        if(comparison > 0) {
          ...
          tally2 += pile;
          System.out.printf("Player 3 wins %d cards%n", pile);
        }

      }
    }
    System.out.printf("Game Over. Player1: %d, Player 2: %d%n", tally1, tally2);
  }
